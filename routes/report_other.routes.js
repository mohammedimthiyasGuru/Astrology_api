...
var report_otherModel = require('./../models/report_otherModel');


router.post('/create', async function(req, res) {
  try{

        await report_otherModel.create({
...
router.get('/deletes', function (req, res) {
      report_otherModel.remove({}, function (err, user) {
          if (err) return res.status(500).send("There was a problem deleting the user.");
             res.json({Status:"Success",Message:"report_otherModel Deleted", Data : {} ,Code:200});     
      });
});


router.post('/getlist_id', function (req, res) {
        report_otherModel.find({Person_id:req.body.Person_id}, function (err, StateList) {
...
router.get('/getlist', function (req, res) {
        report_otherModel.find({}, function (err, Functiondetails) {
